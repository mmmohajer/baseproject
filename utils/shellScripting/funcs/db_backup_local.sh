makeBackupOfDb() {
    ...
    docker exec web_db_1 pg_dump devdb -U devuser > $addr
...
restoreDb() {
    ...
    docker exec web_db_1 psql -U devuser -d devdb -c "DROP SCHEMA public CASCADE; CREATE SCHEMA public;"
    docker exec web_db_1 psql -U devuser -d devdb -f $addr
